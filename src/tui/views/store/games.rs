...
impl Widget for GameListItem {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let title_text = Text::from(self.game.name()).reset().bold();
        ...
        let price_str = format!("{} P", self.game.cost());
        ...
        let desc_text = Paragraph::new(self.game.description())
            .reset()
            .wrap(Wrap::default());
...
impl View for GamesView {
    fn render(&self, frame: &mut Frame, area: Rect, state: &State, tick: u32) {
        ...
        let header = Paragraph::new("Games")
        ...
        for (i, game) in ALL_GAMES.iter().enumerate() {
            let affordable = state.party_points >= game.cost();
            let selected = self.selection == i;
            let num_owned = state.game_token_count(*game);

            let item = GameListItem::new(*game, num_owned, affordable, selected, tick);
            ...
            Action::Select => {
                if let Some(game) = self.selected_game() {
                    ...
                    if state.party_points >= cost {
                        ...
                        state.add_game_token(game);
                        ViewResult::Message(
                            MessageType::Success,
                            format!("Purchased a token for {}!", game.name()),
                        )
                    } else {
                        ViewResult::Message(
                            MessageType::Error,
